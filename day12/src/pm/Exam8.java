...
/*
 쓰레드 사용 2번째 방법
  ...
  3. 앞에서 정의한 클래스를 이용해서 Runnable객체를 생성한다. 
         문제는 Runnable 객체의 내부에는 start()매서드가 존재하지 않기 때문에 
     start()를 가진 Thread 객체를 생성해야 한다. 다만 구현한 run()매서드 자체는
     Runnable객체가 갖고 있으므로 Thread객체를 생성할때  Runnable 객체를
         생성자의 매개변수로 넘겨준다. 이렇게 객체를 생성하면 생성된 Thread 객체 내부의 
     run()매서드는 생성자 매개변수로 넘어온 Runnable 객체 내부의 run()으로 대체된다. 
 */

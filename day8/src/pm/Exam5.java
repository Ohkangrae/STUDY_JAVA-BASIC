...
/*
  ...
  개발자가 명시적으로 타입을 변환해야 한다. 객체도 마찬가지로 타입변환이 일어난다.
  
  객체에서는 자식 클래스에서 부모 클래스쪽으로 변환 되는 것이 업캐스팅
  그 반대는 다운 캐스팅이다.
  객체는 항상 업캐스팅은 할수 있으므로 명시적으로 적어주지 않아도 자바가 대신해준다.
  하지만 객체 명시적으로 적어 줘도 다운캐스팅 자체가 안될 때가 있다.
  다운캐스팅을 수행하면 classCastException 이 발생하고 프로그램이 종료된다.
  ...
  첫번째 human1객체는 사람 클래스로 만들었기 때문에 여기에는 사람의 공통된 속성과 기능들만
  포함되어 있다. 그렇기 때문에 학생으로의 다운캐스팅은 불가능하다.
  반면에 human2는 실제 학생() 생성자로 객체를 생성했으므로 객체 내부에는 사람의 공통된
  특성 뿐이 아니라 학생만의 속성과 기능들이 포함되어 있다. 따라서 human2도 사람 자료형으로 저장돼 있지만
  학생으로 다운캐스팅이 가능하다. 

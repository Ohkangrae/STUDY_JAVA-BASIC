...
/*
 ...
 자바에서는 다양한 종류 클래스와 인터페이스를 제공한다. 이런 클래스 와 인터페이스를
 내부 멤버에서 활용하는 클래스를 작성할때는 제공되는 클래스나 인터페이스의 다양성만큼
 많은 가지수의 클래스를 생성해야 한다. 또한 동일한 이름의 매서드가 다양한 타입의 
 입력 매개변수를 가질수 있또록 하려면 고려하는 입력매개변수 타입의 수만큼 
 오버로딩을 수행해야한다. 그래서 이러한 비효율성을 한번에 해결하는데 필요한 
 문법 요소가 바로 '제네릭' 이다. 
 
  1 제너릭을 사용하지 않고 여러 객체를 저장하는 클래스를 작성

...
값이 추가된 위치의 앞뒤 데이터 정보만 수정하면 된다. 따라서 중간에 데이터를 추가할 때 속도
차이가 날 것이라는 것을 예상 할수 있다. 
그렇다면 장점만 있을까!!! linkedlist는 각 원소가 자신의 인덱스 정보를 갖고있지는 않다. 
특정 인덱스 위치의 값을 가져오기 위해서는 앞에서부터 차례로 번호를 세어가면서 인덱스의 위치를 
찾아야한다. 반면에 ArrayList는 데이터 자제에 인덱스 번호를 갖고 있으므로 특정 인덱스 위치의
데이터를 빠르게 찾을 수 있다. 
데이터를 추가 또는 삭제할때는 LinkedList가 빠르고 데이터를 찾을때는 ArrayList가 빠르다. 
